...
Matrix4f get_R(float alpha, float a, float d, float theta){
	Matrix4f R;
	R(0,0) = cos(theta); R(0,1) = -cos(alpha)*sin(theta); R(0,2) = sin(alpha)*sin(theta); R(0,3) = a*cos(theta);
	R(1,0) = sin(theta); R(1,1) = cos(alpha)*cos(theta); R(1,2) = -sin(alpha)*cos(theta);	R(1,3) = a*sin(theta);
	R(2,0) = 0;	R(2,1) = sin(alpha); R(2,2) = cos(alpha);	R(2,3) = d;
...
Matrix4f get_dR_dtheta(float alpha, float a, float d, float theta){
	Matrix4f R;
	R(0,0) = -sin(theta);	R(0,1) = -cos(alpha)*cos(theta); R(0,2) = sin(alpha)*cos(theta); R(0,3) = -a*sin(theta);
	R(1,0) = cos(theta); R(1,1) = -cos(alpha)*sin(theta);	R(1,2) = sin(alpha)*sin(theta);	R(1,3) = a*cos(theta);	
...
Matrix4f get_dR_dd(float alpha, float a, float d, float theta){
	...
	R(2,0) = 0;	R(2,1) = 0;	R(2,2) = 0;	R(2,3) = 1;
	R(3,0) = 0;	R(3,1) = 0;	R(3,2) = 0;	R(3,3) = 0;
	return R;
}
